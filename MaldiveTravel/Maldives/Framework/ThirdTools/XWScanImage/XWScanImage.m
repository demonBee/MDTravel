...
//        [imageView setFrame:CGRectMake(0.f, 0.f, KScreenWidth, KScreenHeight)];
        //重要！ 将视图显示出来
...
+ (void)pinshAction:(UIPinchGestureRecognizer *)pinch{
    //原始imageview
    UIImageView *imageView = [pinch.view viewWithTag:10];
    pinch.scale=pinch.scale-lastScale+1;
...
+ (void)panAction:(UIPanGestureRecognizer *)pan{
    UIImageView *imageView = [backgroundView viewWithTag:10];
    CGPoint point = [pan translationInView:imageView];
    pan.view.transform = CGAffineTransformTranslate(imageView.transform, point.x, point.y);
