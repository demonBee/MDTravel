...
//    smc.titleFont=[UIFont fontWithName:@".Helvetica Neue Interface" size:14.0f];
    ...
    smc.buttonDownWidth = buttonDownWidth;
...
- (void)AddSegumentArray:(NSArray *)SegumentArray{
    ...
    for (int i = 0; i < SegumentArray.count; i++) {
        ...
        if (i == 0) {
            buttonDownView =[[UIView alloc]initWithFrame:CGRectMake((witdthFloat - self.buttonDownWidth)/2, self.bounds.size.height - 2, self.buttonDownWidth, 2)];
...
-(void)selectTheSegument:(NSInteger)segument{
    ...
    if (selectSeugment != segument) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [buttonDownView setFrame:CGRectMake(segument * witdthFloat + (witdthFloat - self.buttonDownWidth)/2,self.bounds.size.height - 2, self.buttonDownWidth, 2)];
        }];
