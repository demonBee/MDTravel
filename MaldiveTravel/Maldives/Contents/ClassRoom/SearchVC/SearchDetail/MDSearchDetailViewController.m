//
//  MDSearchDetailViewController.m
//  Maldives
//
...
#import "MDSearchDetailViewController.h"
...
@interface MDSearchDetailViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MDSearchDetailViewController
...
- (void)makeUI{
    ...
    [self.segmentControl selectTheSegument:self.status];
    [self.view addSubview:self.segmentControl];
}
- (void)registerCellAndTableViewSet{
    self.allDataArr = [NSMutableArray arrayWithCapacity:0];//0hotel,1Class,2Guide,4Event
    for (int i = 0; i < 4; i++) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.status == 0 || self.status == 3) {//酒店
...
- (void)requestDataWithPage:(NSInteger)page{
    ...
    NSDictionary * pragram = @{@"con":self.keyStr,@"pagen":self.pagen,@"pages":[NSString stringWithFormat:@"%zi",page],@"zt":ztArr[self.status]};
    [[HttpObject manager]getDataWithType:MaldivesType_SEARCH withPragram:pragram success:^(id responsObj) {
